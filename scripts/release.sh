...
case "\${1:-}" in
  ...
  switch)
    ...
    if [[ -z "\$_PROV" ]]; then
      ...
      echo "  Providers:"
      echo "    openai     OpenAI   (default: gpt-5.5)"
      echo "    codex      Codex / compatible API  (default: gpt-5.5)  needs CODEX_API_KEY"
  ...
  model)
    ...
    if [[ -z "\$_MODEL" ]]; then
      ...
      echo "  Examples:"
      echo "    ${CMD} model gpt-5.5              OpenAI flagship"
      echo "    ${CMD} model gpt-5.5-mini         OpenAI faster/cheaper"
